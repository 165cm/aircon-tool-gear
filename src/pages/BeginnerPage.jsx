...
export default function BeginnerPage({ activePage = "beginner-kit", onNavigate }) {
  const recommendedKit = starterKits.find((kit) => kit.recommended) || starterKits[0];
  const recommendedProducts = recommendedKit.productSlugs.map(getProduct).filter(Boolean);
          ...
          <div className="grid gap-6 p-5 md:p-8 lg:grid-cols-[340px_1fr]">
            <aside>
              <div className="grid overflow-hidden rounded-lg border border-metal-300">
                {starterKits.map((kit) => (
                  <div
                    className={`border-b border-metal-200 p-4 last:border-b-0 ${
                      kit.recommended ? "bg-steel text-white" : "bg-white text-navy"
                    }`}
                    key={kit.slug}
                  >
                    <p className="text-sm font-black">{kit.label}</p>
                    <p className={`mt-1 text-xs font-bold ${kit.recommended ? "text-white/75" : "text-metal"}`}>
                      {kit.title}
                    </p>
                    <p className={`mt-2 text-lg font-black ${kit.recommended ? "text-white" : "text-orange"}`}>
                      {kit.price}
                    </p>
                  </div>
                ))}
            ...
            <article className="rounded-lg border border-metal-200 bg-white p-4 shadow-panel">
              <div className="grid gap-5 lg:grid-cols-[1fr_280px]">
                <div>
                  <p className="text-sm font-black text-navy">おすすめ構成: {recommendedKit.label}</p>
                  <img
                    alt=""
                ...
                <div className="flex flex-col justify-end">
                  ...
                  <div className="mt-5 border-t border-metal-200 pt-4">
                    ...
                    <p className="mt-1 text-3xl font-black text-navy">{recommendedKit.price}</p>
                    <p className="mt-2 text-sm font-bold leading-6 text-metal">{recommendedKit.note}</p>
          ...
          <div className="grid border-t border-metal-200 md:grid-cols-3">
            {starterKits.map((kit) => (
              <div className="border-metal-200 p-5 md:border-l first:md:border-l-0" key={kit.slug}>
                <p className="font-black text-navy">{kit.label}</p>
